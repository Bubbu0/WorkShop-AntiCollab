...
![AntiCollab AWS Architecture](/WorkShop-AntiCollab/images/2-Proposal/Diagram.png)
...
- **AWS Secrets Manager & CloudWatch**: Quản lý tập trung các thông tin nhạy cảm (RDS password, JWT, Twilio credentials, Redis AUTH Token, Cognito credentials) được nạp trực tiếp vào ECS Tasks lúc khởi chạy (startup). Đồng thời các tasks ghi log trực tiếp về CloudWatch Logs.
...
- **AWS Cognito**: Quản lý danh tính tập trung, xử lý các luồng đăng ký, đăng nhập và xác thực bảo mật qua JSON Web Tokens (JWT) mà không cần tự lưu trữ mật khẩu ở database phía Backend.

*Luồng hoạt động của hệ thống*:
- **1. Xác thực**: Client (Frontend) gửi thông tin đăng nhập trực tiếp tới **AWS Cognito User Pool**, Cognito xác thực và trả về JSON Web Token (JWT).
- **2. Truy cập ứng dụng**: User truy cập trang web thông qua địa chỉ HTTPS/WSS của **CloudFront**, luồng đi qua tường lửa **AWS WAF** (được cấu hình ở chế độ Monitor mode).
- **3. Phân phối tài nguyên tĩnh**: CloudFront gửi yêu cầu qua cơ chế OAC tới **S3 Frontend** để tải về mã nguồn React app.
- **4. Định tuyến động**: Các yêu cầu API (`/api/*`) hoặc WebSocket (`/ws/*`) được CloudFront chuyển tiếp về **Application Load Balancer (ALB)**. Client gửi kèm token JWT của Cognito trong Header `Authorization`.
- **5. Xử lý ứng dụng**: ALB định tuyến yêu cầu tới **ECS Tasks** trong Private Subnet. Tại đây, Express và SocketAuth Middleware giải mã và kiểm tra tính hợp lệ của token JWT với AWS Cognito public keys. (Khi khởi chạy, ECS Task dùng Task Execution Role để kéo ảnh từ **Amazon ECR** và Task Role để lấy cấu hình từ **AWS Secrets Manager**).
- **6. Đồng bộ cơ sở dữ liệu & Cache**: Task kết nối với **RDS PostgreSQL (Port 5432 SSL)** để truy vấn/lưu dữ liệu và kết nối với **ElastiCache Redis (Port 6379 TLS)** để pub/sub đồng bộ sự kiện Socket.io thời gian thực.
- **7. Tải lên tập tin**: ECS Task tạo Pre-signed URL gửi cho Client, Client tải tệp tin đính kèm trực tiếp lên **S3 Files** thông qua **VPC S3 Gateway Endpoint** nội bộ để đảm bảo bảo mật và tốc độ cao.
- **8. Tín hiệu cuộc gọi thoại**: ECS Task liên kết với **Twilio API** để lấy danh sách máy chủ TURN (ICE Servers) thông qua NAT Gateway và Internet Gateway (IGW) giúp người dùng thiết lập kết nối cuộc gọi voice/video trực tiếp (WebRTC).
...
Dự án được ước tính chi phí hạ tầng hàng tháng tại khu vực us-east-1 theo báo cáo thực tế từ công cụ AWS Pricing Calculator như sau:

*Chi phí hạ tầng hàng tháng (Ước tính)*
- **Amazon ECS Fargate**: 2 Tasks hoạt động liên tục (0.5 vCPU, 1 GB RAM cho mỗi task) **$36.04/tháng**.
- **Amazon RDS (PostgreSQL)**: Lớp instance db.t3.medium, Multi-AZ, 20 GB dung lượng lưu trữ GP3 **$110.45/tháng**.
- **Amazon ElastiCache (Redis)**: Lớp instance cache.t3.micro Multi-AZ (1 replica) phục vụ Pub/Sub và lưu trữ trạng thái **$24.82/tháng**.
- **Application Load Balancer (ALB)**: 1 ALB chính định tuyến lưu lượng Backend **$22.27/tháng**.
- **Amazon CloudFront**: Phục vụ truyền tải tài nguyên tĩnh và bảo mật TLS (Nằm trong Free Tier) **$0.00/tháng**.
- **Amazon S3**: Lưu trữ tập tin & Frontend static files (dung lượng 50 GB S3 Standard) **$1.15/tháng**.
- **AWS Secrets Manager**: Lưu trữ 5 secrets cần thiết cho hệ thống **$2.05/tháng** ($0.40/secret/tháng + phí API gọi tối thiểu).
- **AWS CloudWatch**: Ghi nhận logs của ECS Task và lưu trữ chỉ số **$3.00/tháng** (Ước tính tối thiểu).
- **Mạng và Băng thông**: Truyền tải dữ liệu ra Internet **$2.00/tháng** (Ước tính tối thiểu).

**Tổng cộng chi phí hạ tầng ước tính**: **$201.78/tháng**
